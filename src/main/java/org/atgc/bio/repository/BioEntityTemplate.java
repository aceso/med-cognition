...
public class BioEntityTemplate<T> {

    protected static final Logger log = LogManager.getLogger(BioEntityTemplate.class);
    ...
    private static Map<String, Integer> indexNodeCounts = new HashMap<>();
       ...
       // RestIndex<Node> nodeIndex = null;
        ...
        try {
            ...
            if (pNodeHits.size() > 0) { // if  node already exists
                ...
                indexNodeCounts.put(indexName, indexNodeCounts.getOrDefault(indexName, 0) + 1);
            }
            tx.success();
        } catch (RuntimeException e) {
            tx.failure(); //rollback
    ...
    /**
     * How many existing properties were set.
     * @return
     */
    public static long getPropertySetCount() {
        return propertySetCount;
    }
    ...
    /**
     * How many properties were set per indexName
     * @return
     */
     ...
     * @return
     */
    public static Map getIndexNodeCounts() {
        return indexNodeCounts;
    }
            ...
            if (indexed) {
                if (indexName == null) {
                    ...
                    if (null == nodeIndex.putIfAbsent(node, name, value)) {   // if null, then no previous node exists
                        log.info("ADDED NEW PROPERTY: " + (++propertyCount));
                        log.info("node " + node.getLabels().toString() +  ", id =" + + node.getId() + " name =" + name + " + value =" + value);
                        propertyCounts.put(indexName, propertyCounts.getOrDefault(indexName, 0) + 1);
                    } else {
                        log.info("SET NEW PROPERTY: " + (++propertySetCount));
                        propertySetCounts.put(indexName, propertySetCounts.getOrDefault(indexName, 0) + 1);
